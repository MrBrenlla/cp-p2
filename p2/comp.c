...
void* lectorc(void* aux){
  ...
  printf("----------------Thread lector finalizado\n" );
...
void* escritorc(void* aux){
  ...
  for(int i=0; i<arg->chunks; i++) {
    ...
    printf("----------------Thread escritor finalizado\n" );

}

void* lectord(void* aux){
  chunk ch;
  arge * arg=(arge*) aux;

  for(int i=0; i<chunks(arg->ar); i++) {
      ch = get_chunk(arg->ar, i);
      q_insert(arg->q, ch);
  }
  printf("----------------Thread lector finalizado\n" );
...
void decomp(struct options opt) {
    ...
    pthread_create(&lector,NULL,lectord,(void*)&argl);
    ...
    pthread_create(&escritor,NULL,escritord,(void*)&arge);
